...
public class AxonCalc7
{
    ...
    public int[,] freq0 = new int[1, maxCat];
    public int[,] freq1 = new int[1, maxCat];
    public int[,] freq = new int[1, maxCat];

    public int maxVnow = 1;
    public int totVnow = 1;
    ...
    //DATALABELS
    public string[] BlokLabels = new string[1];  //kc
    ...
    public Int16 Vc;
    public Int16 Vc1;
    public Int16[] NcVc0 = new Int16[1]; //vc0
    public Int16[] NcVc = new Int16[1]; //vc
    public Int16[] NcVc1 = new Int16[1];
    Int16 NcVc0H = 0;
    Int16 NcVcH = 0;
    Int16 NcVc1H = 0;
    ...
    public byte[,] SelectVar = new byte[1, 1]; //vc0 vc
    public byte[] SelectVar1 = new byte[1]; //vc1

    public float[] score = new float[MaxNr];
    public byte[] Hscore = new byte[MaxNr];

    public int blocksLinkedN;
    ...
    public string arraysIniData(Int64 Ns, Int64 Nr, Int64 Vc0, Int64 Vc, Int64 Vc1)
    {
        ...
        try
        {
            ...
            SetVar0 = new Int16[Ns + 1, Vc0 + 1];
            SetVar = new Int16[Nr + 1, Vc + 1];
            SetVar1 = new Int16[Nr + 1,   Vc1 + 1];

            freq0 = new int[Vc0+1, maxCat];
            freq = new int[Vc+1, maxCat];
            freq1 = new int[Vc1+1, maxCat];

            NcVc0 = new Int16[Vc0 + 1]; //vc0
            NcVc = new Int16[Vc + 1]; //vc
            NcVc1 = new Int16[Vc1 + 1];

            //index v0 * v

            posLi = new Int64[maxVnow, maxVnow];
            ...
            SelectVar = new byte[maxVnow, maxVnow]; //vc0 vc
            SelectVar1 = new byte[maxVnow]; //vc1
    ...
    public string arraysIniLinkage(Int64 catN, Int64 cat1N)
    {
        string t = "ok";
        ...
        try
        {
            ...
            ePPratio = new float[cat1N+1 , Vc1 + 1];
    ...
    public void cacheIniCheck()
    {
        ...
        if (NcVc0H >= 20 - 1 || NcVcH >=20 - 1 || Vc0 >= 20 - 1 || Vc >= 20 - 1) { UseProbCache = true; } else { UseProbCache = false; }
        // USE KEY CACHES?
    ...
    public void getCriteria()
    {
        ...
        if (co[1] == "linkcriterium")
        {
            ...
            if (co[2] == "fitsabovethreshold")  //with difference criterium!
            {
                CriteriumHandling = 4;
            }
        }
    ...
    public StringBuilder Probabilities(int iter_n)
    {
            ...
            for (int v0 = 1; v0 <= Vc0; v0++)
            {
                for (int v = 1; v <= Vc; v++)
                {
                    for (int c0 = 1; c0 <= NcVc0[v0]; c0++)
                    {
                        for (int c = 1; c <= NcVc[v]; c++)
                        {
                            ProbCachSaveValue(lcach, lbw, Zero1, 1, v0, v, 0, c0, c, 0); //sPP[i, v0, v]
                            ProbCachSaveValue(lcach, lbw, Zero2, 2, v0, v, 0, c0, c, 0); //sPPn[i, v0, v]
                            ProbCachSaveValue(lcach, lbw, Zero3, 3, v0, v, 0, c0, c, 0); //sPPChi[i, v0, v]
                            ProbCachSaveValue(lcach, lbw, Zero4, 4, v0, v, 0, c0, c, 0); //sPPratio[i, v0, v]
        ...
        for (int s = 1; s < Ns + 1; s++)
        {
            ...
            //Get data of one set         
            Int16[,] SRdata = new Int16[maxRperRecord, totVnow];
            if (!UseVarKeyCache)
            {
                SRdata[0, 0] = Nrr[s];
                ...
                if (SelectVar[v0, v] == 1)
                {
                    ...
                    for (int c0 = 1; c0 < NcVc0[v0] + 1; c0++)
                    {
                        for (int c = 1; c < NcVc[v] + 1; c++)
                        {
                            ...
                            {
                                //HIER
                                //ppr = Convert.ToSingle(observed) / Convert.ToSingle(expected); Ratiox[v0, v] *=  ppr ;
                                ppr =  observed  /  expected ; Ratiox[v0, v] *= ppr;
                                if (IterNumber > 0 && Linkmethod == 0) {
                                    //ppr = (Convert.ToSingle(observed) / Convert.ToSingle(N)) / (Convert.ToSingle(notobserved) / Convert.ToSingle(Nn)) ;   //oplossing voor: DIT GAAT NIET GOED MET CACHE.. WORDT NU NIET MEEGENOMEN I.
                                    ppr = ( observed  /  N ) / ( notobserved  /  Nn );   //oplossing voor: DIT GAAT NIET GOED MET CACHE.. WORDT NU NIET MEEGENOMEN I.
    ...
    public Int64[] SetsGetDelimetedPart1(string rawfileNow, Char dChar)
    {

        int pos = 0; int rpos = 0;  
        ...
        maxVnow = 0;
        if (Vc0 > Vc) { maxVnow = Vc0; } else { maxVnow = Vc; }
        if (maxVnow < Vc1) { maxVnow = Vc1; }
        maxVnow += 1;

        totVnow = Vc0 + Vc + Vc1 + 1;


        BlokLabels = new string[Kc+1];   
        Var0Labels = new string[Vc0 + 1];  
        VarLabels = new string[Vc + 1];   
        ...
        string[,] SetVarI = new string[maxRperRecord, totVnow];

        string[] respnrI = new string[byte.MaxValue];

        NcVc0 = new Int16[Vc0 + 1];
        NcVc = new Int16[Vc + 1];
        NcVc1 = new Int16[Vc1 + 1];
        ...
        do
        {
            ...
            if (i == 0)
            {
                ...
                SetVarI = new string[maxRperRecord, totVnow];
                s += 1; rnNow = 0;
    ...
    public void SetsGetDelimetedPart2(string rawfileNow, char dChar, Int64 Ns, Int64 Nr, int Vc0, int Vc, int Vc1, Int64 rpos)
        ...
        Int64 pos = 0;    Int64 posVarsV = 0; Int64 posVarsK = 0;
        //cacheIniCheck();
        ...
        Int16[,] SRdata = new Int16[Nr, totVnow];
        string[] words =new string[maxVnow];
        //then run again to read the data, NOW WITH CACHE IF NEEDED

        
        StreamReader file = new System.IO.StreamReader(rawfileNow);

        //START POSITIE IS ONTHOUDEN VAN VORIGE KEER!!
        do { t = file.ReadLine(); pos += 1; } while (pos != rpos);
        string[,] SetVarI = new string[maxRperRecord, totVnow];
        string[] respnrI = new string[maxRperRecord];
        Int64 s = 0;   byte rnNow = 0;
         string RecordNr = "";
         Int64 i = 0; Int64 r = 0; t = "";
    ...
    public void SetsWriteDelimeted(System.IO.StreamWriter file, string dChar, bool combine)
    {
        ...
        if (!combine)
        {
            ...
            t = ""; file.WriteLine(t);
            for (int i = 1; i <= Kc; i++) { t = BlokLabels[i]; file.WriteLine(t); }
            t = ""; file.WriteLine(t);
            for (int i = 1; i <= Vc0; i++) { t = Var0Labels[i] + dChar + missingS0[i]; file.WriteLine(t); }
            t = ""; file.WriteLine(t);
            for (int i = 1; i <= Vc; i++) { t = VarLabels[i] + dChar + missingS[i]; file.WriteLine(t); }
            t = ""; file.WriteLine(t);
            for (int i = 1; i <= Vc1; i++) { t = Var1Labels[i] + dChar + missingS1[i]; file.WriteLine(t); }
        ...
        if (!UseVarKeyCache)
        {
            ...
            for (Int64 s = 1; s <= Ns; s++)
            {
                ...
                for (Byte r = 1; r <= Nrr[s]; r++)
                {
                    ...
                    t += RespNr[s].ToString() + dChar.ToString(); ;
    ...
    public StringBuilder LinkRecords(int CriteriumHandling, double ThresHoldCriterium, double ThresHoldPercentageCriterium,   int IterNumber, int Linkmethod, System.IO.StreamWriter fileScores)
    {     
