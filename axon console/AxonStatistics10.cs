...
public class AxonCalc10
{
    ...
    public string arraysIniData(UInt64 Ns, UInt64 Nr, UInt16 Vc0, UInt16 Vc, UInt16 Vc1)
    {
    ...
    public UInt64[] SetsGetDelimetedPart1(System.IO.StreamReader file, Char dChar)
    {
        ...
        Kc = Convert.ToUInt16(words[1]);
        Vc0 = Convert.ToUInt16(words[2]);
        Vc = Convert.ToUInt16(words[3]);
        Vc1 = Convert.ToUInt16(words[4]);
    ...
    public UInt64[] SetsGetDelimetedPart2(System.IO.StreamReader file, char dChar, UInt64 Ns, UInt64 Nr, UInt16 Vc0, UInt16 Vc, UInt16 Vc1, UInt64 rpos)

    {
        ...
        do
        {
            ...
            {   //SAVE TO CHACHE OR TO ARRAY
                ...
                //SRdata = new Int16[maxRperRecordS, maxVar];
                ...
                //for (UInt16 c = 0; c <= Kc; c++)
                //{
                //     SetKey0[s, c] = Convert.ToInt32(words[c + 1]);
                //}
                for (UInt16 v0 = 0; v0 < Vc0; v0++)
                {
                    SetVarI[0, v0] = words[2 + Kc + v0];
                }
            ...
            if (i > 0 & i < maxRperRecordS)
            {
                ...
                for (UInt16 v = 0; v < Vc; v++)
                {
                    ...
                    SetVarI[rnNow, v] = words[2 + Kc + v];
                }
                for (UInt16 v1 = 0; v1 < Vc1; v1++)
                {
                  
                    SetVarI[rnNow, Vc + v1] = words[2 + Kc + Vc + v1];
                }
    ...
    public void SetsWriteDelimeted(System.IO.StreamWriter file, string dChar, bool combine)
    {
        ...
        if (!combine)
        {
            ...
            t = ""; file.WriteLine(t);
            for (UInt16 i = 0; i < Kc; i++) { t = BlokLabels[i]; file.WriteLine(t); }
            t = ""; file.WriteLine(t);
        ...
        for (UInt64 s = 1; s <= Ns; s++)
        {
            ...
            for (UInt16 k = 0; k < Kc; k++) { t += "sk" + k.ToString() + dChar.ToString(); ; }
